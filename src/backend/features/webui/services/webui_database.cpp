...
std::optional<Settings> webui_db_read_first() {
    ...
    sqlite3pp::query qry(db,
                         "SELECT id, kiosk, size_width, size_height, pos_x, "
                         "pos_y FROM webui LIMIT 1;");
    ...
    for (auto row : qry) {
        ...
        spdlog::debug("Found settings with id={}, kiosk={}, width={}, "
                      "height={}, x={}, y={}",
                      settings.id, settings.kiosk, settings.width,
                      settings.height, settings.x, settings.y);
...
std::optional<Settings> webui_db_create(const bool pKiosk, const int64_t pWidth,
    ...
    spdlog::debug(
        "Inserting settings with kiosk={}, width={}, height={}, x={}, y={}",
        pKiosk, pWidth, pHeight, pX, pY);

    sqlite3pp::query qry(db,
                         "INSERT INTO webui (kiosk, size_width, size_height, "
                         "pos_x, pos_y) VALUES (?, ?, ?, ?, ?) RETURNING id, "
                         "kiosk, size_width, size_height, pos_x, pos_y;");
    ...
    for (auto row : qry) {
        ...
        spdlog::debug("Created settings: id={}, kiosk={}, width={}, height={}, "
                      "x={}, y={}",
                      settings.id, settings.kiosk, settings.width,
                      settings.height, settings.x, settings.y);
        return settings;
...
std::optional<Settings> webui_db_update_size(const int64_t pId,
    ...
    sqlite3pp::command cmd(
        db, "UPDATE webui SET size_width = ?, size_height = ? WHERE id = ?;");
...
std::optional<Settings> webui_db_update_pos(const int64_t pId, const int64_t pX,
    ...
    sqlite3pp::command cmd(
        db, "UPDATE webui SET pos_x = ?, pos_y = ? WHERE id = ?;");
